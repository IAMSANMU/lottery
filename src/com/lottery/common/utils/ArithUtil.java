...
public class ArithUtil {
	...
	public static double round2Scale(double v) {
		return round(v, 2);
	}
	 ...
	 * @return
	 */
	public static double sixRound(double amount){
		Double tmpAmount=mul(amount, 1000);
		amount=div(tmpAmount.longValue(),1000);
		
		NumberFormat nfFormat = NumberFormat.getInstance();
		nfFormat.setMaximumFractionDigits(2);
		nfFormat.setGroupingUsed(false);//不进行分组,显示的时候就不会有逗号
		String result= nfFormat.format(amount);
